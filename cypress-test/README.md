...
steps:
  ...
  - uses: pleme-io/actions/cypress-test@v1
    with:
...
|---|---|---|---|
| `config-file` | no | `cypress.config.js` |  |
| `workdir` | no | `.` |  |
...
[`lighthouse-ci`](../lighthouse-ci/) · [`percy-snapshot`](../percy-snapshot/) · [`playwright-test`](../playwright-test/) · [`storybook-deploy`](../storybook-deploy/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.frontend.cypress-test` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction cypress-test ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.frontend.cypress-test
```
