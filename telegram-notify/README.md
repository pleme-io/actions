...
steps:
  ...
  - uses: pleme-io/actions/telegram-notify@v1
    with:
      bot-token: <required>
      chat-id: <required>
      text: <required>
...
[`discord-notify`](../discord-notify/) · [`email-notify`](../email-notify/) · [`matrix-notify`](../matrix-notify/) · [`mattermost-notify`](../mattermost-notify/) · [`pagerduty-notify`](../pagerduty-notify/) · [`slack-notify`](../slack-notify/) · [`teams-notify`](../teams-notify/) · [`twilio-sms`](../twilio-sms/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.comms.telegram-notify` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction telegram-notify ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.comms.telegram-notify
```
