# pleme-io · honeycomb-marker

> Add a Honeycomb marker (release/deploy correlation).
...
steps:
  ...
  - uses: pleme-io/actions/honeycomb-marker@v1
    with:
      api-key: <required>
      dataset: <required>
      message: <required>
...
[`datadog-event`](../datadog-event/) · [`grafana-annotation`](../grafana-annotation/) · [`loki-log-push`](../loki-log-push/) · [`otel-collector-deploy`](../otel-collector-deploy/) · [`prometheus-push`](../prometheus-push/) · [`pyroscope-push`](../pyroscope-push/) · [`sentry-release`](../sentry-release/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.observability.honeycomb-marker` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction honeycomb-marker ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.observability.honeycomb-marker
```
