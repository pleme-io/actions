# pleme-io · velero-backup

> Run velero backup create.
...
steps:
  ...
  - uses: pleme-io/actions/velero-backup@v1
    with:
...
[`argocd-sync`](../argocd-sync/) · [`flux-reconcile`](../flux-reconcile/) · [`helm-deploy`](../helm-deploy/) · [`helmfile-apply`](../helmfile-apply/) · [`k8s-rollout-wait`](../k8s-rollout-wait/) · [`kubectl-apply`](../kubectl-apply/) · [`kustomize-render`](../kustomize-render/) · [`tanka-apply`](../tanka-apply/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.k8s.velero-backup` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction velero-backup ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.k8s.velero-backup
```
