...
steps:
  ...
  - uses: pleme-io/actions/rust-gate@v1
    with:
      run-fmt: "true"
      run-clippy: "true"
      run-test: "true"
...
| `run-clippy` | no | `true` | Run cargo clippy --all-targets -- -D warnings |
| `run-test` | no | `true` | Run cargo test --workspace |
| `rust-toolchain` | no | `stable` | Rust toolchain channel (stable / nightly / 1.89.0) |
...
[`nix-flake-check`](../nix-flake-check/) · [`npm-gate`](../npm-gate/) · [`python-gate`](../python-gate/) · [`tlisp-lint`](../tlisp-lint/) · [`typecheck-gate`](../typecheck-gate/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.validation.rust-gate` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction rust-gate ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.validation.rust-gate
```
