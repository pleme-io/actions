# pleme-io · otel-collector-deploy

> Deploy an OpenTelemetry Collector config to a k8s ConfigMap.
...
steps:
  ...
  - uses: pleme-io/actions/otel-collector-deploy@v1
    with:
...
[`datadog-event`](../datadog-event/) · [`grafana-annotation`](../grafana-annotation/) · [`honeycomb-marker`](../honeycomb-marker/) · [`loki-log-push`](../loki-log-push/) · [`prometheus-push`](../prometheus-push/) · [`pyroscope-push`](../pyroscope-push/) · [`sentry-release`](../sentry-release/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.observability.otel-collector-deploy` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction otel-collector-deploy ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.observability.otel-collector-deploy
```
