# pleme-io · gh-secrets-sync

> Sync GitHub repo/org/env secrets from a typed YAML spec (encrypted).
...
steps:
  ...
  - uses: pleme-io/actions/gh-secrets-sync@v1
    with:
      spec-file: ".github/secrets.yaml"
      scope: "repo"
```
...
|---|---|---|---|
| `spec-file` | no | `.github/secrets.yaml` |  |
| `scope` | no | `repo` |  |
...
[`bandit`](../bandit/) · [`checkov`](../checkov/) · [`conftest`](../conftest/) · [`cosign-verify`](../cosign-verify/) · [`cyclonedx-merge`](../cyclonedx-merge/) · [`gosec`](../gosec/) · [`image-scan`](../image-scan/) · [`kics-scan`](../kics-scan/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.security.gh-secrets-sync` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction gh-secrets-sync ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.security.gh-secrets-sync
```
