# pleme-io · release-promote

> Promote a built artifact between environments (dev → staging → prod). Re-tags an existing image/version rather than rebuilding — ensures bit-identical artifact at each stage.

**Category**: `release-mgmt` — 📦 Release management — Changesets / semantic-release
...
steps:
  ...
  - uses: pleme-io/actions/release-promote@v1
    with:
      source-tag: <required>
      target-tag: <required>
      image: <required>
...
|---|---|---|---|
| `source-tag` | yes | — | Source tag (e.g. dev, staging, v0.1.4) |
| `target-tag` | yes | — | Target tag (e.g. staging, prod) |
| `image` | yes | — | OCI image ref (e.g. ghcr.io/pleme-io/myapp) |
...
## Related primitives — `release-mgmt` category

[`changesets`](../changesets/) · [`release-please`](../release-please/) · [`semantic-release`](../semantic-release/) · [`yank-version`](../yank-version/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.release-mgmt.release-promote` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction release-promote ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.release-mgmt.release-promote
```
