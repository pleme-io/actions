...
(defaction
  "release-promote"
  :description
  "Promote a built artifact between environments (dev → staging → prod) without rebuilding. kind=image-retag re-tags an existing OCI image (bit-identical per stage, docker buildx imagetools). kind=helm-chart-version writes the EXACT chart/image version into the next environment's committed values file as a GitOps commit (typed yq set, fail-loud git-bot commit) — the AUTOBUMP/eclusa FedRAMP chart-version-promotion leg; environments subscribe to the committed exact version, never a moving tag. Push defaults OFF (chain git-push-with-token for the rebase-safe token-rearmed path)."
  :inputs
  ((
     :name "kind"
     :type (:enum (:options ("image-retag" "helm-chart-version")))
     :required nil
     :default "image-retag")
    (
      :name "image"
  ...
  ((:name "promoted")
    ...
    (:name "reason"))
  :behavior
  (:runtime :tatara-script :run-tlisp "release-promote/run.tlisp")
  :semver-compat
  :minor
  :attestation
  :optional)
