# pleme-io · akeyless-injector-validate

> Validate Akeyless sidecar injector annotations on a set of k8s manifests. Sanity-check that secret references point at valid Akeyless paths before applying.
...
steps:
  ...
  - uses: pleme-io/actions/akeyless-injector-validate@v1
    with:
      manifests: <required>
...
|---|---|---|---|
| `manifests` | yes | — | Path or glob to manifest file(s) |
...
[`akeyless-auth`](../akeyless-auth/) · [`akeyless-export-config`](../akeyless-export-config/) · [`akeyless-rotate`](../akeyless-rotate/) · [`akeyless-secret-fetch`](../akeyless-secret-fetch/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.akeyless.akeyless-injector-validate` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction akeyless-injector-validate ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.akeyless.akeyless-injector-validate
```
