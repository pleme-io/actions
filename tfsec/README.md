...
steps:
  ...
  - uses: pleme-io/actions/tfsec@v1
    with:
      workdir: "."
```
...
- **Catalog entry**: `substrate.lib.release.patterns.security.tfsec` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction tfsec ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.security.tfsec
```
