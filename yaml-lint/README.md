...
steps:
  ...
  - uses: pleme-io/actions/yaml-lint@v1
    with:
      paths: "."
```
...
- **Catalog entry**: `substrate.lib.release.patterns.data.yaml-lint` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction yaml-lint ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.data.yaml-lint
```
