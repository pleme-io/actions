...
> Run expo eas build for iOS/Android.
...
steps:
  ...
  - uses: pleme-io/actions/eas-build@v1
    with:
      platform: "all"
      profile: "production"
```
...
[`app-store-connect`](../app-store-connect/) · [`fastlane-deploy`](../fastlane-deploy/) · [`flutter-build`](../flutter-build/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.mobile.eas-build` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction eas-build ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.mobile.eas-build
```
