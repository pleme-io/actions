# pleme-io · pyroscope-push

> Push a profiling sample to a Pyroscope server.
...
steps:
  ...
  - uses: pleme-io/actions/pyroscope-push@v1
    with:
      url: <required>
      app-name: <required>
...
[`datadog-event`](../datadog-event/) · [`grafana-annotation`](../grafana-annotation/) · [`honeycomb-marker`](../honeycomb-marker/) · [`loki-log-push`](../loki-log-push/) · [`otel-collector-deploy`](../otel-collector-deploy/) · [`prometheus-push`](../prometheus-push/) · [`sentry-release`](../sentry-release/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.observability.pyroscope-push` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction pyroscope-push ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.observability.pyroscope-push
```
