# pleme-io · semgrep-scan

> Semgrep SAST scan with configurable rule set.
...
steps:
  ...
  - uses: pleme-io/actions/semgrep-scan@v1
    with:
      config: "auto"
...
[`codeql-scan`](../codeql-scan/) · [`coverage-upload`](../coverage-upload/) · [`k6-load-test`](../k6-load-test/) · [`onepassword-fetch`](../onepassword-fetch/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.uncategorized.semgrep-scan` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction semgrep-scan ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.uncategorized.semgrep-scan
```
