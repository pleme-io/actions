...
> Mirror a local directory tree to S3 with --delete semantics (aws s3 sync).
...
steps:
  ...
  - uses: pleme-io/actions/s3-mirror@v1
    with:
      source: <required>
      bucket: <required>
      key: <required>
...
[`artifact-fetch`](../artifact-fetch/) · [`gcs-sync`](../gcs-sync/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.storage.s3-mirror` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction s3-mirror ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.storage.s3-mirror
```
