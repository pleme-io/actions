...
steps:
  ...
  - uses: pleme-io/actions/npm-publish@v1
    with:
      dry-run: "false"
      access: "public"
```
...
|---|---|---|---|
| `dry-run` | no | `false` | Run as --dry-run only (no upload) |
| `access` | no | `public` | npm publish --access flag (public | restricted) |

## Outputs

| Name | Description |
|---|---|
| `shipped` | true' on success (including skip-when-already), 'false' on failure |
...
- **Catalog entry**: `substrate.lib.release.patterns.npm.npm-publish` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction npm-publish ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.npm.npm-publish
```
