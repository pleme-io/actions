...
> Build a flake output (universal). Optionally pushes to cachix/attic afterward.

**Category**: `nix` — ❄️ Nix — build / cache push
...
steps:
  ...
  - uses: pleme-io/actions/nix-build@v1
    with:
      flake-ref: ".#default"
      system: ""
```
...
## Related primitives — `nix` category

[`nix-attic-push`](../nix-attic-push/) · [`nix-cachix-push`](../nix-cachix-push/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.nix.nix-build` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction nix-build ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.nix.nix-build
```
