# pleme-io · kafka-publish

> Publish a message to a Kafka topic via kcat.

**Category**: `messaging` — 📡 Message brokers — NATS / Kafka
...
steps:
  ...
  - uses: pleme-io/actions/kafka-publish@v1
    with:
      brokers: <required>
      topic: <required>
      message: <required>
...
## Related primitives — `messaging` category
...
- **Catalog entry**: `substrate.lib.release.patterns.messaging.kafka-publish` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction kafka-publish ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.messaging.kafka-publish
```
