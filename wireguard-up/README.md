# pleme-io · wireguard-up

> Bring up a WireGuard tunnel for ephemeral runner access.
...
steps:
  ...
  - uses: pleme-io/actions/wireguard-up@v1
    with:
      config: <required>
```
...
- **Catalog entry**: `substrate.lib.release.patterns.networking.wireguard-up` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction wireguard-up ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.networking.wireguard-up
```
