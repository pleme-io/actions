# pleme-io · nix-attic-push

> Push a built nix path to an Attic binary cache.
...
steps:
  ...
  - uses: pleme-io/actions/nix-attic-push@v1
    with:
      cache-name: <required>
      path: <required>
```
...
[`nix-build`](../nix-build/) · [`nix-cachix-push`](../nix-cachix-push/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.nix.nix-attic-push` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction nix-attic-push ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.nix.nix-attic-push
```
