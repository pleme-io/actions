# pleme-io · atlas-migrate

> Apply schema migrations via Atlas.
...
steps:
  ...
  - uses: pleme-io/actions/atlas-migrate@v1
    with:
      url: <required>
      dir: "migrations/"
```
...
[`db-backup`](../db-backup/) · [`db-migrate`](../db-migrate/) · [`flyway-migrate`](../flyway-migrate/) · [`prisma-migrate`](../prisma-migrate/) · [`sqitch-deploy`](../sqitch-deploy/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.db.atlas-migrate` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction atlas-migrate ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.db.atlas-migrate
```
