# pleme-io · tailscale-auth

> Authenticate runner with Tailscale via OAuth or auth-key.

**Category**: `networking` — 🌐 Networking — WireGuard / Tailscale
...
steps:
  ...
  - uses: pleme-io/actions/tailscale-auth@v1
    with:
      auth-key: <required>
      tags: "tag:ci"
```
...
## Related primitives — `networking` category

[`wireguard-up`](../wireguard-up/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.networking.tailscale-auth` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction tailscale-auth ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.networking.tailscale-auth
```
