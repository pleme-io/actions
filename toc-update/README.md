...
> Auto-update markdown table-of-contents between <!-- toc --> markers. Idempotent — re-runs are no-op when TOC matches headings.
...
steps:
  ...
  - uses: pleme-io/actions/toc-update@v1
    with:
      files: "README.md"
      open-pr: "false"
```
...
|---|---|---|---|
| `files` | no | `README.md` | Space-separated markdown files (or globs) |
| `open-pr` | no | `false` |  |
...
[`api-spec-diff`](../api-spec-diff/) · [`changelog-generate`](../changelog-generate/) · [`docs-publish`](../docs-publish/) · [`docusaurus-build`](../docusaurus-build/) · [`hugo-build`](../hugo-build/) · [`mdbook-build`](../mdbook-build/) · [`mkdocs-build`](../mkdocs-build/) · [`vitepress-build`](../vitepress-build/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.docs.toc-update` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction toc-update ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.docs.toc-update
```
