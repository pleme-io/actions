# pleme-io · buildah-build

> Build an OCI image with buildah (rootless alternative).
...
steps:
  ...
  - uses: pleme-io/actions/buildah-build@v1
    with:
...
[`buildkit-cache-warm`](../buildkit-cache-warm/) · [`crane-mutate`](../crane-mutate/) · [`docker-build-and-push`](../docker-build-and-push/) · [`ko-build`](../ko-build/) · [`oci-image-push`](../oci-image-push/) · [`podman-build`](../podman-build/) · [`skopeo-copy`](../skopeo-copy/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.container.buildah-build` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction buildah-build ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.container.buildah-build
```
