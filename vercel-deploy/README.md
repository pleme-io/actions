...
steps:
  ...
  - uses: pleme-io/actions/vercel-deploy@v1
    with:
      project: <required>
      prod: "false"
```
...
- **Catalog entry**: `substrate.lib.release.patterns.cloud.vercel-deploy` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction vercel-deploy ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.cloud.vercel-deploy
```
