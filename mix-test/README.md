...
steps:
  ...
  - uses: pleme-io/actions/mix-test@v1
    with:
      workdir: "."
```
...
- **Catalog entry**: `substrate.lib.release.patterns.language.mix-test` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction mix-test ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.language.mix-test
```
