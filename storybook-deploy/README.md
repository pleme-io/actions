# pleme-io · storybook-deploy

> Build + deploy a Storybook to gh-pages.
...
steps:
  ...
  - uses: pleme-io/actions/storybook-deploy@v1
    with:
...
|---|---|---|---|
| `output-dir` | no | `storybook-static/` |  |
...
[`cypress-test`](../cypress-test/) · [`lighthouse-ci`](../lighthouse-ci/) · [`percy-snapshot`](../percy-snapshot/) · [`playwright-test`](../playwright-test/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.frontend.storybook-deploy` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction storybook-deploy ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.frontend.storybook-deploy
```
