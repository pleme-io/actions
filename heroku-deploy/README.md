...
steps:
  ...
  - uses: pleme-io/actions/heroku-deploy@v1
    with:
      app: <required>
      api-key: <required>
```
...
- **Catalog entry**: `substrate.lib.release.patterns.cloud.heroku-deploy` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction heroku-deploy ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.cloud.heroku-deploy
```
