...
> Run npm/changesets version + publish flow.
...
steps:
  ...
  - uses: pleme-io/actions/changesets@v1
    with:
      publish: "true"
```
...
[`release-please`](../release-please/) · [`release-promote`](../release-promote/) · [`semantic-release`](../semantic-release/) · [`yank-version`](../yank-version/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.release-mgmt.changesets` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction changesets ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.release-mgmt.changesets
```
