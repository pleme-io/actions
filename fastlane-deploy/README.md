# pleme-io · fastlane-deploy

> Run a fastlane lane to deploy iOS/Android build.
...
steps:
  ...
  - uses: pleme-io/actions/fastlane-deploy@v1
    with:
      lane: <required>
      working-directory: "."
```
...
[`app-store-connect`](../app-store-connect/) · [`eas-build`](../eas-build/) · [`flutter-build`](../flutter-build/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.mobile.fastlane-deploy` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction fastlane-deploy ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.mobile.fastlane-deploy
```
