# pleme-io · nats-publish

> Publish a message to a NATS subject via natscli.
...
steps:
  ...
  - uses: pleme-io/actions/nats-publish@v1
    with:
      server: <required>
      subject: <required>
      message: <required>
...
- **Catalog entry**: `substrate.lib.release.patterns.messaging.nats-publish` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction nats-publish ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.messaging.nats-publish
```
