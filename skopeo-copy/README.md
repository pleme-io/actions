# pleme-io · skopeo-copy

> Copy an OCI image between registries via skopeo copy.
...
steps:
  ...
  - uses: pleme-io/actions/skopeo-copy@v1
    with:
      source: <required>
      dest: <required>
```
...
[`buildah-build`](../buildah-build/) · [`buildkit-cache-warm`](../buildkit-cache-warm/) · [`crane-mutate`](../crane-mutate/) · [`docker-build-and-push`](../docker-build-and-push/) · [`ko-build`](../ko-build/) · [`oci-image-push`](../oci-image-push/) · [`podman-build`](../podman-build/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.container.skopeo-copy` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction skopeo-copy ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.container.skopeo-copy
```
