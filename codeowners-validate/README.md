# pleme-io · codeowners-validate

> Validate .github/CODEOWNERS against repo file tree (catch unowned paths).
...
steps:
  ...
  - uses: pleme-io/actions/codeowners-validate@v1
    with:
      fail-on-unowned: "false"
```
...
[`branch-protect-sync`](../branch-protect-sync/) · [`gh-team-sync`](../gh-team-sync/) · [`stale-issue-bot`](../stale-issue-bot/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.hygiene.codeowners-validate` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction codeowners-validate ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.hygiene.codeowners-validate
```
