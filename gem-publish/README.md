...
steps:
  ...
  - uses: pleme-io/actions/gem-publish@v1
    with:
      gem-name: <required>
...
|---|---|---|---|
| `gem-name` | yes | — | Gem name (matches <gem-name>.gemspec at repo root) |
| `tag-version` | yes | — | Version expected on rubygems (derived from tag) |
...
## Related primitives — `ruby` category
...
- **Catalog entry**: `substrate.lib.release.patterns.ruby.gem-publish` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction gem-publish ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.ruby.gem-publish
```
