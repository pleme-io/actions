...
> Deploy a Fly.io app via flyctl. Uses fly.toml at repo root; honors $FLY_API_TOKEN env var.
...
steps:
  ...
  - uses: pleme-io/actions/fly-deploy@v1
    with:
      app: ""
      strategy: "rolling"
      region: ""
...
|---|---|---|---|
| `app` | no | `` | Fly app name (defaults to fly.toml's [app]) |
| `strategy` | no | `rolling` | Deploy strategy: rolling | immediate | canary | bluegreen |
| `region` | no | `` | Pin deploy to a region |
...
[`aws-assume-role`](../aws-assume-role/) · [`aws-s3-upload`](../aws-s3-upload/) · [`azure-deploy`](../azure-deploy/) · [`cloudflare-pages-deploy`](../cloudflare-pages-deploy/) · [`cloudflare-r2-upload`](../cloudflare-r2-upload/) · [`cloudflare-worker-deploy`](../cloudflare-worker-deploy/) · [`doctl-deploy`](../doctl-deploy/) · [`gcp-auth`](../gcp-auth/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.cloud.fly-deploy` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction fly-deploy ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.cloud.fly-deploy
```
