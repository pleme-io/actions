...
steps:
  ...
  - uses: pleme-io/actions/helm-bump@v1
    with:
      bump-type: "patch"
      skip-when-no-source-changes: "true"
      source-paths: "templates values.yaml Chart.yaml"
```
...
| `source-paths` | no | `templates values.yaml Chart.yaml` | Space-separated globs the skip-detector inspects |
...
[`helm-oci-publish`](../helm-oci-publish/) · [`helm-publish`](../helm-publish/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.helm.helm-bump` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction helm-bump ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.helm.helm-bump
```
