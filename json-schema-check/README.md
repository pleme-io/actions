...
steps:
  ...
  - uses: pleme-io/actions/json-schema-check@v1
    with:
      schema: <required>
      files: <required>
```
...
## Related primitives — `data` category
...
- **Catalog entry**: `substrate.lib.release.patterns.data.json-schema-check` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction json-schema-check ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.data.json-schema-check
```
