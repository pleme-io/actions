# pleme-io · grafana-annotation

> Create a Grafana annotation (release marker, deploy event, incident note). Visible on every dashboard that overlaps the time range.
...
steps:
  ...
  - uses: pleme-io/actions/grafana-annotation@v1
    with:
      url: <required>
      api-token: <required>
      text: <required>
...
|---|---|---|---|
| `url` | yes | — | Grafana base URL (no trailing slash) |
| `api-token` | yes | — |  |
| `text` | yes | — |  |
| `tags` | no | `release` |  |
| `dashboard-id` | no | `` | Scope annotation to a single dashboard (empty = global) |
...
[`datadog-event`](../datadog-event/) · [`honeycomb-marker`](../honeycomb-marker/) · [`loki-log-push`](../loki-log-push/) · [`otel-collector-deploy`](../otel-collector-deploy/) · [`prometheus-push`](../prometheus-push/) · [`pyroscope-push`](../pyroscope-push/) · [`sentry-release`](../sentry-release/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.observability.grafana-annotation` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction grafana-annotation ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.observability.grafana-annotation
```
