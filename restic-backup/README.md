# pleme-io · restic-backup

> Run a restic backup to any supported repo (s3/b2/sftp/etc).

**Category**: `backup` — 🛡️ Backup — restic
...
steps:
  ...
  - uses: pleme-io/actions/restic-backup@v1
    with:
      repository: <required>
      password: <required>
      paths: <required>
...
## Related primitives — `backup` category
...
- **Catalog entry**: `substrate.lib.release.patterns.backup.restic-backup` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction restic-backup ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.backup.restic-backup
```
