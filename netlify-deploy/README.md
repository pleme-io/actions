...
steps:
  ...
  - uses: pleme-io/actions/netlify-deploy@v1
    with:
      site-id: <required>
      dir: "dist/"
      prod: "false"
...
- **Catalog entry**: `substrate.lib.release.patterns.cloud.netlify-deploy` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction netlify-deploy ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.cloud.netlify-deploy
```
