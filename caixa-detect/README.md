# pleme-io · caixa-detect

> Find caixa.tlisp (or any .tlisp file containing (defcaixa ...)) at repo root. Emits the file path + the caixa kind (Biblioteca | Binario | Servico | Supervisor | Aplicacao).
...
steps:
  ...
  - uses: pleme-io/actions/caixa-detect@v1
```
...
| Name | Description |
|---|---|
| `caixa-path` | Path to the .tlisp file containing (defcaixa ...) |
| `caixa-kind` | One of: Biblioteca | Binario | Servico | Supervisor | Aplicacao | unknown |
| `caixa-name` | Symbol name passed as first arg to defcaixa |
...
- **Catalog entry**: `substrate.lib.release.patterns.dispatch.caixa-detect` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction caixa-detect ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.dispatch.caixa-detect
```
