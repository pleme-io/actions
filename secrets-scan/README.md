# pleme-io · secrets-scan

> gitleaks-based secret scan across the repo. Emits typed finding count + severity. Configurable fail-on-found gate.
...
steps:
  ...
  - uses: pleme-io/actions/secrets-scan@v1
    with:
      config-path: ""
      fail-on-found: "true"
```
...
|---|---|---|---|
| `config-path` | no | `` | Path to .gitleaks.toml (optional) |
| `fail-on-found` | no | `true` | Fail when any secret is found |
...
- **Catalog entry**: `substrate.lib.release.patterns.security.secrets-scan` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction secrets-scan ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.security.secrets-scan
```
