...
> Snyk dependency vulnerability scan with severity gate.
...
steps:
  ...
  - uses: pleme-io/actions/snyk-test@v1
    with:
      token: <required>
...
- **Catalog entry**: `substrate.lib.release.patterns.security.snyk-test` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction snyk-test ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.security.snyk-test
```
