# pleme-io · semantic-release

> Run semantic-release (conventional-commits → version).
...
steps:
  ...
  - uses: pleme-io/actions/semantic-release@v1
    with:
      dry-run: "false"
```
...
[`changesets`](../changesets/) · [`release-please`](../release-please/) · [`release-promote`](../release-promote/) · [`yank-version`](../yank-version/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.release-mgmt.semantic-release` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction semantic-release ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.release-mgmt.semantic-release
```
