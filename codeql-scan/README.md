# pleme-io · codeql-scan

> GitHub CodeQL SAST scan. Polymorphic — auto-detects language; uploads SARIF to GitHub Code Scanning.
...
steps:
  ...
  - uses: pleme-io/actions/codeql-scan@v1
    with:
      language: ""
...
|---|---|---|---|
| `language` | no | `` | javascript | python | java | go | csharp | ruby | rust | cpp | actions (empty = autodetect) |
| `queries` | no | `security-extended` | Query suite (security-extended | security-and-quality | empty=default) |
...
[`coverage-upload`](../coverage-upload/) · [`k6-load-test`](../k6-load-test/) · [`onepassword-fetch`](../onepassword-fetch/) · [`semgrep-scan`](../semgrep-scan/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.uncategorized.codeql-scan` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction codeql-scan ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.uncategorized.codeql-scan
```
