# pleme-io · kubectl-apply

> Apply k8s manifests + wait for rollout. Universal — works with any kubectl-reachable cluster.
...
steps:
  ...
  - uses: pleme-io/actions/kubectl-apply@v1
    with:
      manifests: <required>
      namespace: "default"
      wait: "true"
...
|---|---|---|---|
| `manifests` | yes | — | Path or glob to manifest file(s) / dir |
| `namespace` | no | `default` | Target namespace |
| `wait` | no | `true` | Wait for rollout (deployment/statefulset/daemonset) |
| `wait-timeout` | no | `300s` | kubectl rollout status --timeout |
...
[`argocd-sync`](../argocd-sync/) · [`flux-reconcile`](../flux-reconcile/) · [`helm-deploy`](../helm-deploy/) · [`helmfile-apply`](../helmfile-apply/) · [`k8s-rollout-wait`](../k8s-rollout-wait/) · [`kustomize-render`](../kustomize-render/) · [`tanka-apply`](../tanka-apply/) · [`velero-backup`](../velero-backup/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.k8s.kubectl-apply` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction kubectl-apply ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.k8s.kubectl-apply
```
