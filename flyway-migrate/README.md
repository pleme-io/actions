...
steps:
  ...
  - uses: pleme-io/actions/flyway-migrate@v1
    with:
      url: <required>
      user: <required>
      password: <required>
...
[`atlas-migrate`](../atlas-migrate/) · [`db-backup`](../db-backup/) · [`db-migrate`](../db-migrate/) · [`prisma-migrate`](../prisma-migrate/) · [`sqitch-deploy`](../sqitch-deploy/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.db.flyway-migrate` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction flyway-migrate ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.db.flyway-migrate
```
