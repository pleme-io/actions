# pleme-io · cosign-verify

> Verify a cosign signature on an artifact or image.
...
steps:
  ...
  - uses: pleme-io/actions/cosign-verify@v1
    with:
      artifact: <required>
      certificate-identity: <required>
```
...
[`bandit`](../bandit/) · [`checkov`](../checkov/) · [`conftest`](../conftest/) · [`cyclonedx-merge`](../cyclonedx-merge/) · [`gh-secrets-sync`](../gh-secrets-sync/) · [`gosec`](../gosec/) · [`image-scan`](../image-scan/) · [`kics-scan`](../kics-scan/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.security.cosign-verify` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction cosign-verify ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.security.cosign-verify
```
