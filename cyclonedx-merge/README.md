# pleme-io · cyclonedx-merge

> Merge multiple CycloneDX SBOMs into a single combined doc.
...
steps:
  ...
  - uses: pleme-io/actions/cyclonedx-merge@v1
    with:
      inputs-glob: "sbom-*.cyclonedx.json"
      output: "sbom-merged.cyclonedx.json"
```
...
|---|---|---|---|
| `inputs-glob` | no | `sbom-*.cyclonedx.json` |  |
| `output` | no | `sbom-merged.cyclonedx.json` |  |
...
[`bandit`](../bandit/) · [`checkov`](../checkov/) · [`conftest`](../conftest/) · [`cosign-verify`](../cosign-verify/) · [`gh-secrets-sync`](../gh-secrets-sync/) · [`gosec`](../gosec/) · [`image-scan`](../image-scan/) · [`kics-scan`](../kics-scan/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.security.cyclonedx-merge` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction cyclonedx-merge ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.security.cyclonedx-merge
```
