# pleme-io · sentry-release

> Create a Sentry release + associate commits.
...
steps:
  ...
  - uses: pleme-io/actions/sentry-release@v1
    with:
      auth-token: <required>
      org: <required>
      project: <required>
...
[`datadog-event`](../datadog-event/) · [`grafana-annotation`](../grafana-annotation/) · [`honeycomb-marker`](../honeycomb-marker/) · [`loki-log-push`](../loki-log-push/) · [`otel-collector-deploy`](../otel-collector-deploy/) · [`prometheus-push`](../prometheus-push/) · [`pyroscope-push`](../pyroscope-push/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.observability.sentry-release` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction sentry-release ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.observability.sentry-release
```
