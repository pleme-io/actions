# pleme-io · swift-build

> Run swift build on a Swift package.
...
steps:
  ...
  - uses: pleme-io/actions/swift-build@v1
    with:
...
- **Catalog entry**: `substrate.lib.release.patterns.language.swift-build` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction swift-build ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.language.swift-build
```
