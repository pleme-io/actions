...
> Sync a local directory to GCS via gsutil rsync.
...
steps:
  ...
  - uses: pleme-io/actions/gcs-sync@v1
    with:
      source: <required>
      bucket: <required>
      key: <required>
...
[`artifact-fetch`](../artifact-fetch/) · [`s3-mirror`](../s3-mirror/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.storage.gcs-sync` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction gcs-sync ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.storage.gcs-sync
```
