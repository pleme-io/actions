# pleme-io · devcontainer-build

> Build a devcontainer image via @devcontainers/cli.

**Category**: `devx` — 🛠️ Developer experience
...
steps:
  ...
  - uses: pleme-io/actions/devcontainer-build@v1
    with:
      workdir: "."
```
...
## Related primitives — `devx` category
...
- **Catalog entry**: `substrate.lib.release.patterns.devx.devcontainer-build` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction devcontainer-build ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.devx.devcontainer-build
```
