# pleme-io · lighthouse-ci

> Run Lighthouse CI on a URL list + assert score thresholds.
...
steps:
  ...
  - uses: pleme-io/actions/lighthouse-ci@v1
    with:
      urls: <required>
      score-threshold: "90"
```
...
[`cypress-test`](../cypress-test/) · [`percy-snapshot`](../percy-snapshot/) · [`playwright-test`](../playwright-test/) · [`storybook-deploy`](../storybook-deploy/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.frontend.lighthouse-ci` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction lighthouse-ci ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.frontend.lighthouse-ci
```
