# pleme-io · vault-fetch

> Fetch a secret from HashiCorp Vault via JWT-OIDC auth.
...
steps:
  ...
  - uses: pleme-io/actions/vault-fetch@v1
    with:
      vault-url: <required>
      role: <required>
...
- **Catalog entry**: `substrate.lib.release.patterns.security.vault-fetch` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction vault-fetch ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.security.vault-fetch
```
