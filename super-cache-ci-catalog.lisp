...
(defaction
  "sui-service-up"
  :description
  "Resolve, health-check, and export the sui service (sui-daemon-graph) endpoint + selected store/cache/sandbox tiers for a super-cache-ci build. Does not own daemon lifecycle."
  :inputs
  ((
     :name "mode"
     :type (:enum (:options ("connect" "ephemeral")))
     :required nil
     :default "ephemeral")
    (
      :name "endpoint"
  ...
  ((:name "endpoint")
    ...
    (:name "reason"))
  :behavior
  (:runtime :tatara-script :run-tlisp "sui-service-up/run.tlisp")
  :semver-compat
  :minor
  :attestation
  :optional)
...
(defaction
  ...
  ((:name "cache-hit")
    ...
    (:name "outputs"))
  :behavior
  (:runtime :tatara-script :run-tlisp "super-cache-restore/run.tlisp")
  :semver-compat
  :minor
  :attestation
  :optional)
...
(defaction
  "super-cache-save"
  :description
  "Persist a build's outputs to the durable super-cache tiers, write-if-absent, content-addressed, no lock (the eliminate-the-shared-cell pattern)."
  :inputs
  ((
     :name "endpoint"
     :type :string
  ...
  ((:name "saved")
    ...
    (:name "reason"))
  :behavior
  (:runtime :tatara-script :run-tlisp "super-cache-save/run.tlisp")
  :semver-compat
  :minor
  :attestation
  :optional)
...
(defaction
  "bigorna"
  :description
  "CI ENTRY: set up a buildx builder with NATIVE per-arch nodes (no QEMU) + a sui-store cache front, in one step, so an unchanged `docker buildx build` afterward runs native-multi-arch + warm. Thin keyway front over the shipped `bigorna setup` binary (sui-bigorna): renders a typed BigornaConfig YAML from the inputs, hands it to `bigorna setup --config`, forwards the JSON receipt. Runner's own arch is native for free; a foreign arch is native only with an arch-endpoints entry, else emulated (refused under fallthrough=error, excluded under native-subset)."
  :inputs
  ((
     :name "platforms"
     :type :string
    ...
    (
      :name "fallthrough"
      :type (:enum (:options ("error" "native-subset")))
      :required nil
      :default "error")
    (
      :name "builder-name"
    ...
    (
      :name "driver"
      :type (:enum (:options ("docker-container" "kubernetes" "remote")))
      :required nil
      :default "docker-container")
    (
      :name "bigorna-version"
      :type :string
      :required nil
      :default "main"))
  :outputs
  ((:name "builder")
    ...
    (:name "result"))
  :behavior
  (:runtime :tatara-script :run-tlisp "bigorna/run.tlisp")
  :semver-compat
  :minor
  :attestation
  :optional)
...
(defaction
  "nix-image"
  :description
  "Build native-arch nix OCI image tarballs via dockerTools (NO Dockerfile, NO QEMU), one per arch, resolving the flake attr from a typed {base}/{arch}/{svc} template — covers substrate mkImageReleaseApp (dockerImage-<arch>), mkGoDockerImage multi-service (dockerImage-<arch>-<svc>), and a colon-namespaced image flake (dockerImage:<arch>:<svc>). Fan out over runs-on:[self-hosted,<arch>] for a native build. Routes through the sui super-cache when SUI_ENDPOINT is set (LiveTODO:super-cache-build); correct local nix build otherwise."
  :inputs
  ((
     :name "image-attr"
     :type :string
  ...
  ((:name "tarball-amd64")
    ...
    (:name "result"))
  :behavior
  (:runtime :tatara-script :run-tlisp "nix-image/run.tlisp")
  :semver-compat
  :minor
  :attestation
  :optional)
...
(defaction
  "ghcr-publish"
  :description
  "Push a nix OCI image tarball to a private registry (ghcr.io/pleme-io/<svc>) under the AUTOBUMP exact tag <arch>-r<run>-<sha>, plus an optional moving <arch>-latest human pointer (never a deploy source). Auth is a pre-condition (docker/login-action first)."
  :inputs
  ((
     :name "repo"
     :type :string
  ...
  ((:name "tag")
    ...
    (:name "result"))
  :behavior
  (:runtime :tatara-script :run-tlisp "ghcr-publish/run.tlisp")
  :semver-compat
  :minor
  :attestation
  :optional)
...
(defaction
  "zot-pull-scan"
  :description
  "The zot faucet gate: pull an image from a private zot registry, scan the PULLED bytes with Trivy, and admit it into the trusted zone ONLY when the scan clears the fail-on-severity gate. Nothing enters the trusted zone unscanned."
  :inputs
  ((:name "ref" :type :string :required t)
    (
      :name "fail-on-severity"
      :type (:enum (:options ("LOW" "MEDIUM" "HIGH" "CRITICAL" "none")))
      :required nil
      :default "HIGH")
    (
      :name "ignore-unfixed"
  ...
  ((:name "admitted")
    ...
    (:name "result"))
  :behavior
  (:runtime :tatara-script :run-tlisp "zot-pull-scan/run.tlisp")
  :semver-compat
  :minor
  :attestation
  :optional)
...
(defaction
  ...
  :description
  "Daemon-aware sui-dockerfile-wrapper GHA integration point (supa-charge-akeyless-ci Phase 3b): probes the node-local sui-dockerfile-node-cache-daemon Unix socket, routes through DaemonAwareCacheClient when present, falls through cleanly to the direct-to-remote-tier Phase 2 wrapper when absent."
  :inputs
  ((:name "dockerfile-path" :type :string :required t)
    (
      :name "build-context"
      :type :string
      :required nil
  ...
  ((:name "daemon-used")
    ...
    (:name "duration-ms"))
  :behavior
  (:runtime :rust-binary-crate-install :crate "sui-dockerfile-wrapper" :bin "gha-entrypoint")
  :semver-compat
  :minor
  :attestation
  :required) ;; ── private-Zot delivery + multi-arch manifest + FedRAMP attest leg ── ;; The three verbs below are authored as CO-LOCATED defactions (each ;; ships its own <name>/defaction.lisp with the same tier-honest header), ;; matching the tameshi-attest / super-cache-build precedent — a ;; duplicated row here would be the exact drift CATALOG-REFLECTION ;; forbids. This is the POINTER (not a copy) so a reader of the seed ;; finds them; folding both the co-located entries and this seed into the ;; canonical repo-forge/pleme-actions-catalog is the shared TataraScript ;; `ActionBehavior`-variant LiveTODO named in every header: ;; ;;   zot-push            → zot-push/defaction.lisp ;;     push a nix OCI tarball to the PRIVATE in-cluster Zot under the ;;     AUTOBUMP exact tag <arch>-r<run>-<sha>; report the pushed digest. ;;     (Sibling of ghcr-publish; FedRAMP images NEVER go to ghcr.io.) ;;   manifest-list-join  → manifest-list-join/defaction.lisp ;;     compose per-arch digests into ONE multi-arch OCI index at ;;     r<run>-<sha>; report the index digest (the exact deploy coord). ;;   cartorio-attest     → cartorio-attest/defaction.lisp ;;     the FedRAMP three-pillar receipt: BLAKE3 chain link + Nix-closure ;;     SBOM (CycloneDX) + SLSA v1.0 provenance. Sibling of tameshi-attest ;;     (the build receipt), sharing the BLAKE3 core (stdlib hash:*). ;; ── per-package materializability gate leg (the cheap pre-derive gate) ── ;; Also a CO-LOCATED defaction (ships its own ferrite-check/defaction.lisp ;; with the same tier-honest header) — POINTER not copy, per the block ;; above: ;; ;;   ferrite-check       → ferrite-check/defaction.lisp ;;     for ONE package (a flake image attr) verify it is MATERIALIZABLE (the ;;     attr resolves to a derivation via a cheap `nix eval`, NOT a derive) ;;     BEFORE the expensive build, content-address its SOURCE (the output ;;     store-path), and emit a PoMS — a Proof-of-Materialization-Spec ;;     receipt (schema pleme-io.ferrite.materialization-spec/v1) — cached by ;;     that source hash so a re-run over unchanged source is a pure cache ;;     HIT (skip re-eval + derive). Single-responsibility sibling of ;;     build-matrix (the (svc,arch) fan) + gen-build-spec (the spec ;;     freshness gate); it gates one fan cell. Sibling of tameshi-attest / ;;     cartorio-attest on ONE chain (carries chain.prev, shares the BLAKE3 ;;     core stdlib hash:*).
