...
steps:
  ...
  - uses: pleme-io/actions/python-gate@v1
    with:
      run-fmt: "true"
      run-lint: "true"
      run-test: "true"
...
|---|---|---|---|
| `run-fmt` | no | `true` | Run ruff format --check |
| `run-lint` | no | `true` | Run ruff check |
| `run-test` | no | `true` | Run pytest |
...
[`nix-flake-check`](../nix-flake-check/) · [`npm-gate`](../npm-gate/) · [`rust-gate`](../rust-gate/) · [`tlisp-lint`](../tlisp-lint/) · [`typecheck-gate`](../typecheck-gate/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.validation.python-gate` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction python-gate ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.validation.python-gate
```
