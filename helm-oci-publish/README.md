...
steps:
  ...
  - uses: pleme-io/actions/helm-oci-publish@v1
    with:
      chart-path: <required>
      registry: <required>
      version: ""
...
|---|---|---|---|
| `chart-path` | yes | — | Path to the Helm chart directory (containing Chart.yaml) |
| `registry` | yes | — | OCI registry URL (oci:// scheme) |
| `version` | no | `` | Override chart version; empty falls through to Chart.yaml's version |
| `lib-chart-dir` | no | `` | External library chart directory for file:// deps |
| `lib-chart-name` | no | `pleme-lib` | Library chart name |
...
[`helm-bump`](../helm-bump/) · [`helm-publish`](../helm-publish/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.helm.helm-oci-publish` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction helm-oci-publish ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.helm.helm-oci-publish
```
