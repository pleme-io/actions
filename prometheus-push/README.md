# pleme-io · prometheus-push

> Push metrics to a Prometheus pushgateway. Useful for emitting deploy/release counters from CI.
...
steps:
  ...
  - uses: pleme-io/actions/prometheus-push@v1
    with:
...
|---|---|---|---|
| `gateway-url` | yes | — | Pushgateway base URL |
| `job` | yes | — |  |
| `metrics` | yes | — | Multi-line text in pushgateway exposition format |
...
[`datadog-event`](../datadog-event/) · [`grafana-annotation`](../grafana-annotation/) · [`honeycomb-marker`](../honeycomb-marker/) · [`loki-log-push`](../loki-log-push/) · [`otel-collector-deploy`](../otel-collector-deploy/) · [`pyroscope-push`](../pyroscope-push/) · [`sentry-release`](../sentry-release/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.observability.prometheus-push` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction prometheus-push ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.observability.prometheus-push
```
