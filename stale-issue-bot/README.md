# pleme-io · stale-issue-bot

> Mark stale issues + close after threshold.
...
steps:
  ...
  - uses: pleme-io/actions/stale-issue-bot@v1
    with:
      days-stale: "60"
      days-close: "14"
...
[`branch-protect-sync`](../branch-protect-sync/) · [`codeowners-validate`](../codeowners-validate/) · [`gh-team-sync`](../gh-team-sync/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.hygiene.stale-issue-bot` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction stale-issue-bot ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.hygiene.stale-issue-bot
```
