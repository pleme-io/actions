...
steps:
  ...
  - uses: pleme-io/actions/sqitch-deploy@v1
    with:
      target: "db:pg:"
```
...
[`atlas-migrate`](../atlas-migrate/) · [`db-backup`](../db-backup/) · [`db-migrate`](../db-migrate/) · [`flyway-migrate`](../flyway-migrate/) · [`prisma-migrate`](../prisma-migrate/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.db.sqitch-deploy` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction sqitch-deploy ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.db.sqitch-deploy
```
