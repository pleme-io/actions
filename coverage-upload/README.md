# pleme-io · coverage-upload

> Generate test coverage + upload to Codecov. Polymorphic — detects ecosystem (rust uses cargo-tarpaulin, npm uses jest --coverage, python uses pytest --cov).

**Category**: `uncategorized` — 🔧 Uncategorized — needs a home
...
steps:
  ...
  - uses: pleme-io/actions/coverage-upload@v1
    with:
      codecov-token: ""
      fail-without-token: "false"
```
...
|---|---|---|---|
| `codecov-token` | no | `` | Codecov upload token (for private repos; public repos do tokenless upload) |
| `fail-without-token` | no | `false` | Fail when no token is provided AND the repo is private |
...
## Related primitives — `uncategorized` category

[`codeql-scan`](../codeql-scan/) · [`k6-load-test`](../k6-load-test/) · [`onepassword-fetch`](../onepassword-fetch/) · [`semgrep-scan`](../semgrep-scan/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.uncategorized.coverage-upload` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction coverage-upload ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.uncategorized.coverage-upload
```
