...
steps:
  ...
  - uses: pleme-io/actions/playwright-test@v1
    with:
      project: <required>
      workdir: "."
```
...
[`cypress-test`](../cypress-test/) · [`lighthouse-ci`](../lighthouse-ci/) · [`percy-snapshot`](../percy-snapshot/) · [`storybook-deploy`](../storybook-deploy/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.frontend.playwright-test` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction playwright-test ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.frontend.playwright-test
```
