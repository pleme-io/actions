...
steps:
  ...
  - uses: pleme-io/actions/airflow-trigger@v1
    with:
      base-url: <required>
      dag-id: <required>
      conf: "{}"
...
[`temporal-trigger`](../temporal-trigger/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.workflow.airflow-trigger` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction airflow-trigger ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.workflow.airflow-trigger
```
