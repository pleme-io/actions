# pleme-io · argocd-sync

> Trigger argocd app sync + wait for Healthy/Synced. Sibling of flux-reconcile.
...
steps:
  ...
  - uses: pleme-io/actions/argocd-sync@v1
    with:
      app-name: <required>
      server: <required>
      auth-token: <required>
...
|---|---|---|---|
| `app-name` | yes | — |  |
| `server` | yes | — | argocd server URL (with --grpc-web if needed) |
| `auth-token` | yes | — |  |
| `timeout` | no | `300` |  |
...
[`flux-reconcile`](../flux-reconcile/) · [`helm-deploy`](../helm-deploy/) · [`helmfile-apply`](../helmfile-apply/) · [`k8s-rollout-wait`](../k8s-rollout-wait/) · [`kubectl-apply`](../kubectl-apply/) · [`kustomize-render`](../kustomize-render/) · [`tanka-apply`](../tanka-apply/) · [`velero-backup`](../velero-backup/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.k8s.argocd-sync` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction argocd-sync ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.k8s.argocd-sync
```
