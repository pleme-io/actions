# pleme-io · temporal-trigger

> Start a Temporal workflow via tctl/temporal CLI.

**Category**: `workflow` — ⚙️ Workflow orchestration — Temporal / Airflow
...
steps:
  ...
  - uses: pleme-io/actions/temporal-trigger@v1
    with:
      address: <required>
...
## Related primitives — `workflow` category
...
- **Catalog entry**: `substrate.lib.release.patterns.workflow.temporal-trigger` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction temporal-trigger ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.workflow.temporal-trigger
```
