# pleme-io · onepassword-fetch

> Fetch a secret from 1Password via Service Account token. Sibling of akeyless-secret-fetch.
...
steps:
  ...
  - uses: pleme-io/actions/onepassword-fetch@v1
    with:
...
|---|---|---|---|
| `service-account-token` | yes | — |  |
| `reference` | yes | — | 1Password secret ref (e.g. op://vault/item/field) |
| `output-env` | no | `` | Env var to export to subsequent steps |
...
[`codeql-scan`](../codeql-scan/) · [`coverage-upload`](../coverage-upload/) · [`k6-load-test`](../k6-load-test/) · [`semgrep-scan`](../semgrep-scan/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.uncategorized.onepassword-fetch` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction onepassword-fetch ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.uncategorized.onepassword-fetch
```
