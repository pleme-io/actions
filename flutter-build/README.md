# pleme-io · flutter-build

> Build a Flutter app for a target.
...
steps:
  ...
  - uses: pleme-io/actions/flutter-build@v1
    with:
      target: "apk"
      flavor: ""
```
...
[`app-store-connect`](../app-store-connect/) · [`eas-build`](../eas-build/) · [`fastlane-deploy`](../fastlane-deploy/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.mobile.flutter-build` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction flutter-build ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.mobile.flutter-build
```
