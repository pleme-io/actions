...
steps:
  ...
  - uses: pleme-io/actions/prisma-migrate@v1
    with:
      workdir: "."
```
...
[`atlas-migrate`](../atlas-migrate/) · [`db-backup`](../db-backup/) · [`db-migrate`](../db-migrate/) · [`flyway-migrate`](../flyway-migrate/) · [`sqitch-deploy`](../sqitch-deploy/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.db.prisma-migrate` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction prisma-migrate ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.db.prisma-migrate
```
