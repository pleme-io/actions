# pleme-io · branch-protect-sync

> Apply branch-protection rules from a JSON spec.

**Category**: `hygiene` — 🧹 Repo hygiene
...
steps:
  ...
  - uses: pleme-io/actions/branch-protect-sync@v1
    with:
      spec-file: ".github/branch-protection.json"
      branch: "main"
```
...
|---|---|---|---|
| `spec-file` | no | `.github/branch-protection.json` |  |
| `branch` | no | `main` |  |
...
## Related primitives — `hygiene` category

[`codeowners-validate`](../codeowners-validate/) · [`gh-team-sync`](../gh-team-sync/) · [`stale-issue-bot`](../stale-issue-bot/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.hygiene.branch-protect-sync` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction branch-protect-sync ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.hygiene.branch-protect-sync
```
