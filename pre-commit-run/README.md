...
steps:
  ...
  - uses: pleme-io/actions/pre-commit-run@v1
    with:
      all-files: "true"
```
...
- **Catalog entry**: `substrate.lib.release.patterns.devx.pre-commit-run` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction pre-commit-run ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.devx.pre-commit-run
```
