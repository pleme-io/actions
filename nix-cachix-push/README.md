# pleme-io · nix-cachix-push

> Push a built nix path to a Cachix binary cache.
...
steps:
  ...
  - uses: pleme-io/actions/nix-cachix-push@v1
    with:
      cache-name: <required>
      path: <required>
```
...
[`nix-attic-push`](../nix-attic-push/) · [`nix-build`](../nix-build/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.nix.nix-cachix-push` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction nix-cachix-push ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.nix.nix-cachix-push
```
