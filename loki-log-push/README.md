# pleme-io · loki-log-push

> Push a batch of log lines to a Loki ingester.
...
steps:
  ...
  - uses: pleme-io/actions/loki-log-push@v1
    with:
      url: <required>
      labels: "{"app":"ci"}"
      lines-file: <required>
...
[`datadog-event`](../datadog-event/) · [`grafana-annotation`](../grafana-annotation/) · [`honeycomb-marker`](../honeycomb-marker/) · [`otel-collector-deploy`](../otel-collector-deploy/) · [`prometheus-push`](../prometheus-push/) · [`pyroscope-push`](../pyroscope-push/) · [`sentry-release`](../sentry-release/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.observability.loki-log-push` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction loki-log-push ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.observability.loki-log-push
```
