...
steps:
  ...
  - uses: pleme-io/actions/pulumi-up@v1
    with:
      stack: <required>
      workdir: "."
```
...
[`iac-forge`](../iac-forge/) · [`terraform-apply`](../terraform-apply/) · [`terraform-plan`](../terraform-plan/)


## Sources

- **Action source**: [`action.yml`](./action.yml) + [`run.tlisp`](./run.tlisp)
- **Catalog entry**: `substrate.lib.release.patterns.iac.pulumi-up` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction pulumi-up ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.iac.pulumi-up
```
