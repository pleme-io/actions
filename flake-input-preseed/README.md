...
tree.** So a heavy git input (e.g. the big-monorepo 18,904-commit
...
big-monorepo source). Once the CA path is local, the in-flake locked
eval **reuses it and skips the clone** (measured: in-flake `drvPath` eval
21s→14s, zero big-monorepo clone). No `--override-input` needed.
...
| `inputs` | `""` | space/comma list of flake-input **node** names to preseed (`big-monorepo`) |
