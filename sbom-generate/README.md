...
steps:
  ...
  - uses: pleme-io/actions/sbom-generate@v1
    with:
...
|---|---|---|---|
| `output-format` | no | `cyclonedx-json` | cyclonedx-json | spdx-json | syft-json |
| `output-file` | no | `sbom.cyclonedx.json` | Path to write the SBOM |

## Outputs

| Name | Description |
|---|---|
| `sbom-path` | Path of the written SBOM |
...
- **Catalog entry**: `substrate.lib.release.patterns.security.sbom-generate` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction sbom-generate ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.security.sbom-generate
```
