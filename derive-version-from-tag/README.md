...
|---|---|---|---|
| `ref-name` | no | `${{ github.ref_name }}` | Tag ref name (default: github.ref_name) |

## Outputs

| Name | Description |
|---|---|
| `version` | Derived version (ref-name with leading 'v' stripped); set by run.tlisp via $GITHUB_OUTPUT |
...
- **Catalog entry**: `substrate.lib.release.patterns.gh.derive-version-from-tag` —
  [patterns-full.nix](https://github.com/pleme-io/substrate/blob/main/lib/release/patterns-full.nix)
- **Future typed source**: `(defaction derive-version-from-tag ...)` per
...
```bash
nix eval --raw github:pleme-io/substrate#lib.aarch64-darwin.release.patterns.gh.derive-version-from-tag
```
